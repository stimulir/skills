...
One refusal in that family is a consult, not a cap.
`eval_derive_candidate_rejected` means the ledger already holds a verdict on
this exact candidate, and it arrives with the reason and the date. The
correct output is still a report: quote it, name the producing run, and end
the invocation. The difference is what comes next. A cap refusal has no
override; this one has `--allow-rejected`, and only a human may order it.
